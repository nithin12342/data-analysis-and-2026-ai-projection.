...
def run_simulation(params=None):
    ...
    market_val = global_df["compute_capacity_index"] * global_df["price_index"] * 32.0
    initial_val = market_val.iloc[0] if len(market_val) > 0 else 1.0
    history["marketValuation"] = interpolate_to_quarters(market_val)
    history["indexVal"] = interpolate_to_quarters(100.0 * (market_val / initial_val))
